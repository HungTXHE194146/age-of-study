...
import { joinClassByCode, getStudentClass, getClassSubjects } from "@/lib/classService";
import type { StudentWithClass } from "@/types/class";
import { motion, AnimatePresence } from "framer-motion";
import { Users, CheckCircle, AlertCircle, BookOpen, ArrowRight } from "lucide-react";
...
export default function LearnPage() {
  ...
  // Class state
  const [currentClass, setCurrentClass] = useState<StudentWithClass | null>(null);
        ...
        {currentClass?.class ? (
          ...
          <motion.div
            ...
            className={`relative bg-gradient-to-br from-emerald-500 via-green-500 to-teal-600 rounded-2xl p-8 hover:shadow-2xl transition-all group overflow-hidden ${
              currentClass?.class?.class_code ? 'cursor-pointer' : 'cursor-not-allowed opacity-75'
            }`}
          >
                  ...
                  <div className="space-y-2 text-white/90">
                    ...
                    {currentClass.class.homeroom_teacher_name && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="font-semibold">{currentClass.class.homeroom_teacher_name}</span>
                      </div>
                    )}
                    
                    <div className="flex items-center gap-2 text-sm">
                      <span className="opacity-80">📚 Năm học:</span>
                      <span className="font-semibold">{currentClass.class.school_year}</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="mb-8">
          ...
          {loadingSubjects ? (
            ...
            <div className="bg-gray-50 rounded-xl p-8 text-center">
              ...
              <p className="text-gray-600">Chưa có môn học nào được phân công</p>
            </div>
          ) : (
            <div className="grid gap-6 md:grid-cols-3">
              {classSubjects.map((subject, index) => {
                ...
                const colors = [
                  { bg: 'bg-red-100', text: 'text-red-600', hover: 'hover:bg-red-600', border: 'border-red-200' },
                  { bg: 'bg-blue-100', text: 'text-blue-600', hover: 'hover:bg-blue-600', border: 'border-blue-200' },
                  { bg: 'bg-green-100', text: 'text-green-600', hover: 'hover:bg-green-600', border: 'border-green-200' },
                  { bg: 'bg-purple-100', text: 'text-purple-600', hover: 'hover:bg-purple-600', border: 'border-purple-200' },
                  { bg: 'bg-orange-100', text: 'text-orange-600', hover: 'hover:bg-orange-600', border: 'border-orange-200' },
                  { bg: 'bg-teal-100', text: 'text-teal-600', hover: 'hover:bg-teal-600', border: 'border-teal-200' },
                ];
                ...
                return (
                  ...
                  >
                    <div className="text-center">
                      <div className={`w-16 h-16 ${color.bg} rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform`}>
                      ...
                      <p className="text-gray-600 text-sm mb-4">
                        {subject.teachers.map(t => t.full_name).filter(Boolean).join(', ') || 'Chưa có giáo viên'}
                      </p>
                      <button className={`px-6 py-2 ${color.bg} ${color.text} rounded-full font-semibold ${color.hover} hover:text-white transition-colors`}>
                        Bắt đầu
                      </button>
                    </div>
                  </motion.div>
