              ...
              {selectedNodeId ? (
                nodeStatsLoading ? (
                  ...
                  <div className="space-y-6">
                    <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                      <h3 className="text-indigo-300 font-semibold mb-3">Thông số tiến độ</h3>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-700/50 text-center">
                          <div className="text-2xl font-bold text-green-400">{nodeStats?.stats?.completed || 0}</div>
                          <div className="text-xs text-slate-400">Đã hoàn thành</div>
                        </div>
                        <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-700/50 text-center">
                          <div className="text-2xl font-bold text-amber-400">{nodeStats?.stats?.needsReview || 0}</div>
                          <div className="text-xs text-slate-400">Cần ôn tập</div>
                        </div>
                      </div>
                    </div>
                          ...
                          nodeStats.tests.map((t) => (
                            <div key={t.id} className="bg-slate-900 p-3 rounded-lg border border-slate-700 hover:border-indigo-500/50 cursor-pointer transition-all">
                              ...
                              <div className="text-xs text-slate-400 mt-1 flex justify-between">
                                <span>{t.type === 'practice' ? 'Luyện tập' : 'Kiểm tra'} ({t.status === 'completed' ? 'Hoàn thành' : t.status === 'in_progress' ? 'Đang làm' : 'Chưa làm'})</span>
                                {t.score !== null && <span className="text-green-400">{t.score} điểm</span>}
                              </div>
                            </div>
                          ))
                        ) : (
                          <div className="text-sm text-slate-500 italic text-center py-2">Bạn chưa có bài kiểm tra nào trong bài học này.</div>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-700 text-center">
                  ...
                  <p className="text-slate-400 text-sm">
                    Bấm vào một Node bất kỳ trên cây kỹ năng để xem thống kê tiến độ và danh sách bài Test của bạn cho bài học đó.
                  </p>
                </div>
              )}
            </div>
