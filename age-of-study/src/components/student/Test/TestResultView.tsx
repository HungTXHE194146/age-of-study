...
export default function TestResultView({
    ...
    return (
        <div className="max-w-6xl mx-auto">
            ...
            >
                <h1 className={`text-4xl font-black uppercase tracking-tight mb-2 ${grade.color}`}>
                    {grade.label}
                </h1>
                ...
                <div className="space-y-8">
                    {submissionResult.questions.map((question, index) => (
                        <QuestionReviewItem
                            ...
                            index={index}
                            question={question}
                            userAnswer={submissionResult.answers.find((a) => a.question_id === question.id)}
                        />
                    ))}
                </div>
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between">
                <div>
                    <p className="text-sm font-black uppercase text-slate-500 mb-1">{label}</p>
...
function QuestionReviewItem({ index, question, userAnswer }: { index: number; question: Question; userAnswer: any }) {
    ...
    return (
        <div
            className={`p-6 md:p-8 rounded-3xl border-4 shadow-[4px_4px_0_0_rgba(0,0,0,1)] ${isCorrect ? "border-green-400 bg-green-50/50" : "border-red-400 bg-red-50/50"
                }`}
        >
            <div className="flex items-start justify-between mb-6">
                <div className="flex-1">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <span
                            className={`px-4 py-1.5 rounded-xl border-2 border-slate-800 font-black shadow-[2px_2px_0_0_rgba(0,0,0,1)] transform rotate-2 ${isCorrect ? "bg-green-400 text-slate-900" : "bg-red-400 text-white"
                                }`}
                        >
    ...
    switch (qType) {
        ...
        case "MATCHING":
            const studentPairs = userAnswer?.text_answer
                ? (JSON.parse(userAnswer.text_answer) as Array<{ left: string; right: string }>)
                : [];
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {studentPairs.length > 0 ? (
                                studentPairs.map((pair, i) => {
                                    const isMatchCorrect = correctPairs.some((cp: any) => cp.left === pair.left && cp.right === pair.right);
                                    return (
                                        <div
                                            key={i}
                                            className={`p-3 rounded-xl border-2 flex items-center justify-between gap-2 font-bold ${isMatchCorrect ? "bg-green-50 border-green-300 text-green-700" : "bg-red-50 border-red-300 text-red-700"
                                                }`}
                                        >
                                            <span>{pair.left}</span>
                                            <span>↔</span>
        ...
        case "CATEGORIZATION":
            const studentCats = userAnswer?.text_answer
                ? (JSON.parse(userAnswer.text_answer) as Array<{ name: string; items: string[] }>)
                : [];
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {studentCats.length > 0 ? (
                                studentCats.map((cat, i) => {
                                    const correctCat = correctCats.find((cc: any) => cc.name === cat.name);
                                    return (
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {cat.items.map((item, j) => {
                                                    ...
                                                    return (
                                                        <span
                                                            key={j}
                                                            className={`px-2 py-1 rounded-lg border font-bold text-sm ${isItemInCat ? "bg-green-100 border-green-400 text-green-700" : "bg-red-100 border-red-400 text-red-700"
                                                                }`}
                                                        >
                                                            {item}
                                                        </span>
        ...
        case "FIND_ERROR":
            ...
            const fullText =
                question.content?.metadata?.sentence || question.content?.questionText || question.content?.question || "";
            ...
            const getSnappedEnd = (end: number) => {
                let e = end;
                while (e < fullText.length - 1 && fullText[e + 1] !== " " && fullText[e + 1] !== "\n") e++;
                return e;
            };
                ...
                <div className="space-y-4">
                    <div className="bg-white p-5 rounded-2xl border-2 border-slate-300">
                        ...
                        {studentError ? (
                            <p className="text-lg font-bold">
                                <span
                                    className={`px-2 py-1 rounded border-2 ${isCorrect ? "bg-green-100 border-green-400 text-green-700" : "bg-red-100 border-red-400 text-red-700"
                                        }`}
                                >
                                    {studentError}
                                </span>
                    ...
                    {!isCorrect && (
                        <div className="bg-green-50 p-5 rounded-2xl border-2 border-green-400">
                            <p className="text-sm font-bold uppercase text-green-700 mb-2">Đáp án đúng:</p>
                            <p className="text-lg font-bold">
                                Sửa lỗi: <span className="text-red-600 line-through">{correctErrorPart || "..."}</span>
                                {" → "}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white p-5 rounded-2xl border-2 border-slate-300">
                        ...
                        <div className="p-3 bg-slate-50 border-2 border-dashed border-slate-300 rounded-lg whitespace-pre-wrap font-bold text-lg text-slate-800">
                            {userAnswer?.text_answer || <span className="text-red-500 italic">Em đã không để lại câu trả lời</span>}
                        </div>
                    </div>
                        ...
                        {!isCorrect && (
                            <div className="bg-green-100 p-5 rounded-2xl border-2 border-green-400">
                                ...
                                <p className="font-bold text-lg text-green-900">
                                    {String.fromCharCode(65 + (question.correct_option_index ?? 0))}.{" "}
                                    {(() => {
                                        const opt = (question.content.options || [])[question.correct_option_index ?? 0];
                                        return typeof opt === "string" ? opt : opt?.text;
                                    })() || ""}
                                </p>
                            </div>
