...
export const QuestionEditor = memo(function QuestionEditor({
  ...
  const handleTrueFalseContentChange = (
    field: "true" | "false",
    value: string,
  ) => {
    setEditedQuestion((prev) => ({
  ...
  const validateForm = (): boolean => {
    ...
    if (editedQuestion.type === "MULTIPLE_CHOICE") {
      const hasEmptyOptions = editedQuestion.options.some(
        (opt) => !opt.text.trim(),
      );
      if (hasEmptyOptions) {
    ...
    if (editedQuestion.type === "TRUE_FALSE") {
      const trueOption = editedQuestion.options.find(
        (opt) => opt.label === "A",
      );
      const falseOption = editedQuestion.options.find(
        (opt) => opt.label === "B",
      );
    ...
    if (editedQuestion.type === "WORD_ORDERING") {
      if (
        !editedQuestion.metadata?.orderedWords ||
        editedQuestion.metadata.orderedWords.length < 2
      ) {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-800 font-bold mt-1">
                Câu {editedQuestion.number} -{" "}
                <span className="bg-yellow-200 px-2 py-0.5 border-2 border-black rounded shadow-[2px_2px_0_0_rgba(0,0,0,1)] text-sm">
          ...
          <div className="pl-6 space-y-8 relative z-10">
            ...
            <div className="bg-indigo-50 p-4 rounded-xl border-2 border-indigo-200 shadow-[4px_4px_0_0_rgba(99,102,241,0.2)]">
              <label className="text-lg font-black text-indigo-900 mb-3 block uppercase tracking-tight flex items-center gap-2">
                ...
                <span className="text-xs font-bold bg-indigo-200 px-2 py-0.5 rounded text-indigo-700 normal-case">
                  Xuất hiện khi trả lời sai
                </span>
              </label>
              <textarea
            ...
            <div className="bg-yellow-50 p-4 rounded-xl border-2 border-yellow-200 shadow-[4px_4px_0_0_rgba(254,240,138,0.2)]">
              <label className="text-lg font-black text-yellow-900 mb-3 block uppercase tracking-tight flex items-center gap-2">
                ...
                <span className="text-xs font-bold bg-yellow-200 px-2 py-0.5 rounded text-yellow-700 normal-case">
                  Xuất hiện sau khi nộp bài
                </span>
              </label>
              <textarea
            ...
            <div className="bg-white p-4 rounded-xl border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
              ...
              <select
                ...
                onChange={(e) =>
                  handleDifficultyChange(e.target.value as QuestionDifficulty)
                }
              ...
              <div className="space-y-6 bg-white p-6 rounded-xl border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {editedQuestion.options.map((option, index) => (
                    <div
                      key={option.id}
                      className={`p-4 rounded-xl border-2 transition-all shadow-[2px_2px_0_0_rgba(0,0,0,1)] space-y-3 ${
                        option.isCorrect
                          ? "bg-green-50 border-green-600 ring-2 ring-green-600 ring-offset-1"
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <label className="flex items-center gap-3">
                          <div
                            className={`w-8 h-8 rounded-full border-2 font-black flex items-center justify-center ${
                              option.isCorrect
            ...
            {editedQuestion.type === "TRUE_FALSE" && (
              <div className="space-y-6 bg-white p-6 rounded-xl border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
                <h3 className="text-xl font-black text-gray-900 uppercase tracking-tight pb-4 border-b-2 border-dashed border-gray-300">
                  Nội dung Đúng/Sai
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="p-4 rounded-xl border-2 border-black bg-green-50 shadow-[2px_2px_0_0_rgba(0,0,0,1)] space-y-3">
                    <label className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full border-2 border-black font-black flex items-center justify-center bg-green-400 text-black">
                        A
                      </div>
                    ...
                    <input
                      value={
                        editedQuestion.options.find((opt) => opt.label === "A")
                          ?.text || ""
                      }
                      onChange={(e) =>
                        handleTrueFalseContentChange("true", e.target.value)
                      }
                      className="w-full px-3 py-2 border-2 border-dashed border-gray-400 rounded-lg focus:outline-none focus:border-black font-medium transition-colors bg-white"
                      placeholder="VD: Đúng"
                    />
                  ...
                  <div className="p-4 rounded-xl border-2 border-black bg-red-50 shadow-[2px_2px_0_0_rgba(0,0,0,1)] space-y-3">
                    <label className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full border-2 border-black font-black flex items-center justify-center bg-red-400 text-black">
                        B
                      </div>
                      <span className="font-bold text-gray-900">
                        Đáp án SAI
                    ...
                    <input
                      value={
                        editedQuestion.options.find((opt) => opt.label === "B")
                          ?.text || ""
                      }
                      onChange={(e) =>
                        handleTrueFalseContentChange("false", e.target.value)
                      }
                      className="w-full px-3 py-2 border-2 border-dashed border-gray-400 rounded-lg focus:outline-none focus:border-black font-medium transition-colors bg-white"
                      placeholder="VD: Sai"
                    />
            ...
            {editedQuestion.type === "WORD_ORDERING" && (
              <div className="bg-white p-6 rounded-xl border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
                ...
                <input
                  value={
                    editedQuestion.metadata?.orderedWords?.join(", ") || ""
                  }
                  onChange={(e) =>
            ...
            {editedQuestion.type === "MATCHING" && (
              <div className="bg-white p-6 rounded-xl border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
                <h3 className="text-xl font-black text-gray-900 mb-3 block uppercase tracking-tight pb-4 border-b-2 border-dashed border-gray-300">
                  Các cặp nối
                </h3>
                <div className="space-y-4">
                  {(editedQuestion.metadata?.matchingPairs || []).map(
                    (pair, idx) => (
                  ...
                  <Button
                    onClick={() =>
                      handleMetadataChange("matchingPairs", [
                        ...(editedQuestion.metadata?.matchingPairs || []),
                        { left: "", right: "" },
                      ])
                    }
            ...
            {editedQuestion.type === "FIND_ERROR" && (
              <div className="bg-white p-6 rounded-xl border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] space-y-4">
                <h3 className="text-xl font-black text-gray-900 uppercase tracking-tight pb-4 border-b-2 border-dashed border-gray-400">
                  Nội dung làm bài (Câu chứa lỗi)
                </h3>
                <textarea
                  value={(editedQuestion.metadata as any)?.sentence || ""}
                  onChange={(e) =>
                    handleMetadataChange("sentence", e.target.value)
                  }
                  rows={2}
                ...
                <div className="space-y-2">
                  ...
                  <div className="p-4 bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl flex flex-wrap gap-2">
                    {((editedQuestion.metadata as any)?.sentence || "")
                      .split(/(\s+)/)
                      .map((part: string, idx: number, arr: string[]) => {
                        ...
                        const isSelected =
                          editedQuestion.metadata?.errorPosition?.startIndex ===
                            currentPos &&
                          editedQuestion.metadata?.errorPosition?.endIndex ===
                ...
                <div>
                  <label className="text-xs font-bold uppercase text-gray-500">
                    Nội dung sửa lại đúng
                  </label>
                  <input
                    value={
                      editedQuestion.metadata?.errorPosition?.correctText || ""
                    }
                    onChange={(e) =>
                      handleMetadataChange("errorPosition", {
                        ...editedQuestion.metadata?.errorPosition,
            ...
            {editedQuestion.type === "FILL_IN_BLANKS" && (
              <div className="bg-white p-6 rounded-xl border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] space-y-4">
                ...
                <h3 className="text-lg font-black text-gray-800 pt-4">
                  Đáp án các ô trống
                </h3>
                {(editedQuestion.metadata?.blanks || []).map(
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    const nextIndex = (editedQuestion.metadata?.blanks || [])
                      .length;
                    handleMetadataChange("blanks", [
                      ...(editedQuestion.metadata?.blanks || []),
                      { index: nextIndex, answer: "" },
                    ]);
                  }}
              ...
              <div className="bg-white p-6 rounded-xl border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] space-y-4">
                <div className="flex justify-between items-center pb-4 border-b-2 border-dashed border-gray-400">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      handleMetadataChange("categories", [
                ...
                {(editedQuestion.metadata?.categories || []).map(
                  (cat: any, idx: number) => (
                    <div
                      key={idx}
                      className="p-4 bg-gray-50 border-2 border-black rounded-xl space-y-3"
                    >
