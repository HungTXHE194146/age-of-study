...
export function canAccessRoute(userRole: Profile['role'], routePath: string): boolean {
...
export function getRedirectPath(userRole: Profile['role'], routePath: string): string | null {
  const route = findRoute(routeConfig, routePath);
  if (!route) {
    return null;
  }
